...
class BvgSensor(Entity):
    ...
    def __init__(
        ...
        self.file_name = "bvg_{}_{}.json".format(name, stop_id)
    ...
    def fetchDataFromURL(self):
        try:
            with urlopen(self.url) as response:
                ...
                try:
                    with open("{}{}".format(self.file_path, self.file_name), mode="w+", encoding="utf-8") as fd:
                    ...
                    _LOGGER.error(
                        "Could not write MY file. Please check your configuration and read/write access for path:{}".format(
                            self.file_path
                        )
                    )
    ...
    def getSingleConnection(self, direction, min_due_in, nmbr):
        ...
        for dest in direction:
            for pos in self.data:
                _LOGGER.debug("conf_direction: {} pos_direction {}".format(direction, pos['direction']))
                # if pos['direction'] in direction:
                if pos["direction"] in dest:
                    if pos["when"] is None:
                        continue
